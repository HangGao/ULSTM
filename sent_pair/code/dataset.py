...
class SICKDataset(data.Dataset):
    ...
    def read_sentence(self, line):
        indices = self.vocab.convertToIdx(line.split(), UNK_WORD)
...
class MSRPDataset(data.Dataset):
    def __init__(self, path=None, vocab=None):
        super(MSRPDataset, self).__init__()

        if path is not None and vocab is not None:
            self.vocab = vocab
    ...
    def add_item(self, item):
        ltree, lsent, rtree, rsent, label = item
        self.lsentences.append(lsent)
        self.rsentences.append(rsent)

        self.ltrees.append(ltree)
        self.rtrees.append(rtree)
    ...
    def read_labels(self, filename):
        with open(filename, 'r') as f:
            labels = list(map(lambda x: int(x), f.readlines()))
            labels = torch.Tensor(labels)
        return labels


def read_data(task, data_dir, vocab):
    if task == 'sick' or task == 'sts' or task == 'sick_addit':
        return SICKDataset(data_dir, vocab)
    elif task == 'msrp':
        return MSRPDataset(data_dir, vocab)
    else:
