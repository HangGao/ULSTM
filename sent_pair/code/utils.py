...
def map_label_to_tensor(label, num_classes):
    target = torch.LongTensor(1)
    target[0] = int(label)
    return target

def normalize_label(label, min_val=1., max_val=5.):
    nlabel = (label - min_val) / (max_val - min_val)
    return nlabel

def scale_label(label, min_val=1., max_val=5.):
    # scale labels back to [min_val, max_val]
    nlabel = (label * (max_val - min_val)) + min_val
    return nlabel
