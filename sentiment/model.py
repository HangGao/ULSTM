...
class PLSTM_Layer(nn.Module):
    ...
    def forward(self, X, hidden=None):
        ...
        for i in range(seq_len):
            ...
            u = torch.mul(self.zc.expand(batch_size, self.hidden_size), hidden_c)
            z = F.tanh(z + u)
...
class Network(nn.Module):
    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, num_classes, dropout=0.5, dropouth=0.5, dropouti=0.5, dropoute=0.1, wdrop=0):
        ...
        self.rnn = RNNModel(rnn_type, ntoken, ninp, nhid, nlayers, dropout, dropouth, dropouti, dropoute, wdrop, tie_weights=False, use_decoder=False)
